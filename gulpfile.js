...
gulp.task('dev', function() {
  ...
  new WebpackDevServer(webpack(webpackDevConfig), {
    ...
    headers: {
      "Access-Control-Allow-Origin": config.endpoints.web,
      "Access-Control-Allow-Credentials": true
    },
    proxy: {
      '*': config.endpoints.web,
    }
  }).listen(config.ports.webpack, config.host, function(err) {
      if (err) throw new gutil.PluginError("webpack-dev-server", err);
      gutil.log("[webpack-dev-server]", config.endpoints.proxy_webpack + "/webpack-dev-server/");
  });
});
...
gulp.task('client-dev', function() {
  var cmd = new run.Command('NODE_ENV=development webpack --config ./config/webpack.config.dev.js');
  cmd.exec();
});
